package Capability::TermExtraction::TermEx;

# script obviously klunged from
# termex-1.49/SampleScripts/UNIX/ex_EPT.pl

require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw (TermexTermExtraction);

use lib "/var/lib/myfrdcsa/sandbox/termex-1.49/termex-1.49";
use TermExtract::EnglishPlainText;

use File::Temp;

sub TermexTermExtraction {
  my %args = @_;
  return unless $args{String};

  my $data = new TermExtract::EnglishPlainText;
  my $f = File::Temp->new;
  print $f $args{String};
  my $fn = $f->filename;
  my $InputFile = $f;		# 入力ファイル指定
