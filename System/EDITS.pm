...
sub init {
  ...
  $self->EDITSDir($args{EDITSDir} || "/var/lib/myfrdcsa/sandbox/edits-1.0/edits-1.0");
...
sub StartServer {
  ...
  if (! -f ConcatDir($self->EDITSDir,$modelfile)) {
    ...
    my $c1 = "export EDITS_PATH=\"/var/lib/myfrdcsa/sandbox/edits-1.0/edits-1.0\" && cd $editsdir && bin/edits -train -conf=share/configurations/conf2.xml $devfile $modelfile";
...
sub RTE {
  ...
  $fh->close;
  my $annotator = $args{Annotator} || $self->Annotator;
  my $c1 = "export EDITS_PATH=\"/var/lib/myfrdcsa/sandbox/edits-1.0/edits-1.0\" && cd $editsdir && bin/annotate -annotator=$annotator $inputfile $annotatedfile";
  ApproveCommands
    (
     Commands => [$c1],
     Method => "parallel",
  ...
  my $c2 = "export EDITS_PATH=\"/var/lib/myfrdcsa/sandbox/edits-1.0/edits-1.0\" && cd $editsdir && bin/edits -test ".
    "-conf=share/configurations/conf2.xml -model=$modelfile $annotatedfile -overwrite -full=$outputfile";
  print "$c2\n";
  system $c2;
